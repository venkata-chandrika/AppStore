...
class AppStore extends Component {
  ...
  render() {
    const {inputSearch, activeTabId} = this.state
    const filteredAppList = this.getFilteredAppList()
    const searchAppResult = filteredAppList.filter(each =>
        ...
        <ul className="tab-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              onSelectedTabItem={this.onSelectedTabItem}
              isActive={eachTab.tabId === activeTabId}
            />
          ))}
        </ul>
        <ul className="app-item-container">
          {searchAppResult.map(eachList => (
